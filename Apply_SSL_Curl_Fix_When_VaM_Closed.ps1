...
$pending = Join-Path $dir 'AllPackagesLinker.dll.pending_ssl_curl'
$log = Join-Path $dir 'apply_ssl_curl_fix.log'
...
$bak = Join-Path $dir ("AllPackagesLinker.dll.bak_" + (Get-Date -Format 'yyyyMMdd_HHmmss') + "_before_ssl_curl_apply")

...
- 1.4.7：选择场景后会提前识别每个 Person 的性别，并为每名女性 Person 提供独立的“启动外观”选择行；候选来自收藏的本地/VAR 女性 Appearance 预设，默认保持原样。加载前只合并外观相关 storables（包含 Hair），保留场景原姿势、插件和动画；VAR 预设中的 `SELF:/` 会改写为来源包 UID。
- 1.4.7：VR 镜头模式的左摇杆 Y 轴由升降改为围绕当前头显世界位置前后翻转，支持连续越过 180°且头显枢轴位置不漂移；X 轴仍为水平转向，设置中可分别调整转向与翻转方向、速度。
- 1.4.6：场景加载开始时取消与 VaM 核心 P24 重叠的 APL 皮肤预热；加载期间延迟服装/头发浏览器缩略图，并按纹理签名合并并发重复请求，场景完成后再恢复全部原始 UI 回调。四人场景冷测观察到 196-288 次请求仅涉及 18 个唯一资源，因此可从加载关键路径移除 178-270 次重复工作；Harmony 前缀和同步代码保持 VaM 旧 Mono 兼容。
- 1.4.5：场景加载前会把当前 VAR 自身的 `SELF:/Custom/Scripts/...` 引用直接本地化；避免场景收尾阶段再次 Reload 同一脚本并重复初始化服装/网格。FNAF Foxy 冷加载日志中的 6 次补丁 Reload（约 13 秒额外开销）由此移除。
- 1.4.4：精简模式主加载完成后显示“补充加载 N 个 Atom”；按钮使用 `SuperController.LoadMerge()` 只合并本次延迟的 Atom，不重新加载当前人物，并在主场景仍加载时保持禁用。
- 1.4.3：原“人物优先”收紧并更名为“全人物”：只保留场景中的全部 Person（不区分男性/女性）和 4 类 VaM 必需系统 Atom；灯光、UI、CUA、音频及其他独立 Atom 全部延迟，可进入场景后再点“加载其余 Atom”。
- 1.4.2：通过 Harmony 接通当前 VaM `v1.1` 加速核心中已有但为空实现的 `LoadCo` 性能埋点；只在 APL 场景加载 profile 活跃时记录阶段汇总，过滤逐 Atom 明细，不覆盖 `Assembly-CSharp.dll`，用于继续定位 VaM 本体的冷加载瓶颈。
...
- `全人物`：先加载全部 Person（不区分男性/女性）和 VaM 必需系统 Atom；其他独立 Atom 全部延后。
- `极简人物`：先只加载主角、系统 Atom 和灯光。

场景包含多个人物时，可用主角按钮循环选择 Person。精简模式加载成功后，详情区会显示“补充加载 N 个 Atom”；点击后只通过 `LoadMerge` 合并本次延迟的 Atom，不会重新加载当前人物。开始加载另一个场景或主场景加载失败时，这个待合并状态会自动清除。
